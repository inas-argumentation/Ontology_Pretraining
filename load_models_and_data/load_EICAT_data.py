...
def load_evidence_sentences(processed_data):
    if os.path.exists(data_dir("datasets/EICAT_dataset/full_text_evidence.json")):
        with open(data_dir("datasets/EICAT_dataset/full_text_evidence.json"), "r") as f:
...
def load_paper_full_text(paper_id):
    with open(data_dir(f"datasets/EICAT_dataset/EICAT_papers_json/{paper_id}.json")) as f:
        content = json.load(f)
...
def load_dataset(tokenizer, return_label_dict=False):
    with open(data_dir("datasets/EICAT_dataset/EICAT_fulltext_dataset_final.json"), "r") as f:
        data = json.load(f)
